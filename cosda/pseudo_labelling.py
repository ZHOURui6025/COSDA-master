...
def get_pseudo_label(args, model, dataloader, feature_batch=None, targets=None, new_epoch= True, all_proto=None, test=False):
    if args.pl_method == 'Topk3_distance':
        return Topk_distance3(args, model, dataloader, feature_batch=feature_batch, targets=targets, new_epoch=new_epoch, all_proto=all_proto, test=test)
    else:
...
def Topk_distance3(args, model, dataloader, feature_batch=None, targets=None, new_epoch=True, all_proto=None, test=False):
    ...
    if new_epoch:
        ...
        sorted_pred_cls, sorted_pred_cls_idxs = torch.sort(pred_cls_bank, dim=0, descending=True) #dim=
        ...
        psd_label_prior = torch.zeros(feat_proto_simi.shape).scatter(1, psd_label_prior_idxs.unsqueeze(1).cpu(), 1.0).cuda() # one_hot prior #[N  #
