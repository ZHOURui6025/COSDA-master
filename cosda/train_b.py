...
def train(args, epoch, best_dict, source_train_dataloader, target_train_dataloader, len_target, len_source, optimizer, logger, epoch_idx, memory_source_features, memory_source_labels):
    ...
    for batch_idx in range(max(len_target, len_source)):
        optimizer.zero_grad()
        print(batch_idx, len_target, len_source)
        try:
    ...
    gc.collect()  # 垃圾回收
    torch.cuda.empty_cache()  # 清空 GPU 缓存（如果用 GPU）
...
if __name__ == "__main__":
    ...
    args.logger = set_logger(args, log_name=args.backbone_arch+"log_target_training.txt")
    args.logger.info(
                args.note
            )
    param_group = []
    ...
    len_source, len_target = len(source_dataloader), len(target_train_dataloader)

    # source_loader_iter = ContinuousDataloader(source_dataloader)
    # target_loader_iter = ContinuousDataloader(target_train_dataloader)
    # test_loader_iter = ContinuousDataloader(target_test_dataloader)
    ...
    for epoch_idx in tqdm(range(args.epochs), ncols=60):
        ...
        memory_source_features, memory_source_labels, best_dict = train(args, epoch_idx, best_dict , source_dataloader, target_train_dataloader, len_target, len_source, optimizer, args.logger, epoch_idx, memory_source_features, memory_source_labels)
