...
def train(args, model, dataloader, criterion, optimizer, epoch_idx=0.0):
    ...
    for imgs_train, _, imgs_label, batch_idx in tqdm(dataloader, ncols=60):
        ...
        loss_s = ce_criterion(args, imgs_train, y_s, domain='source')
        
        lr_scheduler(optimizer, iter_idx, iter_max)
        optimizer.zero_grad()
        loss = sum(loss for loss in loss_s.values())
        loss.backward()
        optimizer.step()
...
def main(args):
    ...
    if args.checkpoint is not None and os.path.isfile(args.checkpoint):
        ...
        save_dir = os.path.join(this_dir, "checkpoints_osda", args.dataset, "source_{}".format(args.s_idx),
    ...
    logger = set_logger(args, log_name="log_pretraining.txt")
    ...
    for idx in range(len(args.targetlist_domain_dir_list)):
        ...
        target_data_list = open(os.path.join(targetlist_data_dir, "osda_list.txt"), "r").readlines()
        target_dataset = COSDAataset(args, target_data_dir, target_data_list, d_type="target", preload_flg=False)
    ...
    for epoch_idx in tqdm(range(args.epochs), ncols=60):
        ...
        checkpoint_file = "source_checkpoint.pth"
        torch.save({
            "epoch":epoch_idx,
