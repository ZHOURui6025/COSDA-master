...
# class COSDA_HEAD(nn.Module):
#     def __init__(self, args):
#
#         super(COSDA_HEAD, self).__init__()
#
#         # self.backbone_arch = args.backbone_arch   # resnet50
#         self.embed_feat_dim = args.embed_feat_dim # 256
#         self.class_num = args.class_num           # shared_class_num + source_private_class_num
...
#
#         self.backbone_feat_dim = self.clip.visual_projection.out_features
#         self.feat_embed_layer = Embedding(self.backbone_feat_dim, self.embed_feat_dim, type=args.bn_type)
#         self.intervener = MLP(self.embed_feat_dim, self.embed_feat_dim, args)
#         self.class_layer = Classifier(self.embed_feat_dim, self.class_num, type='wn')
#
#     def get_embed_feat(self, input_imgs):
#         # input_imgs [B, 3, H, W]
#         backbone_feat = self.image_encoder(pixel_values=input_imgs).last_hidden_state
#         pooled_features = backbone_feat[:, 0, :]
#         # embed_feat = self.feat_embed_layer(backbone_feat)
#         return pooled_features
#
#     def forward(self, x, apply_softmax=True):
#         rois = self.image_encoder(pixel_values=x).last_hidden_state
#         rois = rois[:, 0, :]
#         rois_c, v = self.feat_embed_layer(rois)
#         y = self.class_layer(rois_c, apply_softmax)
#
...
# class COSDA_CLIP(nn.Module):
#     def __init__(self, args):
#
#         super(COSDA_CLIP, self).__init__()
#
#         # self.backbone_arch = args.backbone_arch   # resnet50
#         self.embed_feat_dim = args.embed_feat_dim # 256
#         self.class_num = args.class_num           # shared_class_num + source_private_class_num
#         clip_model, _ = clip.load(args.clip_model_name, device="cpu", jit=False)
#         if args.training:
#             clip_model.load_state_dict(torch.load(args.clip_model_name))
#
#         # 图像编码器(直接使用CLIP的视觉部分)
...
#                     param.requires_grad = True
#
#         self.backbone_feat_dim = self.image_encoder.proj.shape[1]
#         self.feat_embed_layer = Embedding(self.backbone_feat_dim, self.embed_feat_dim, type=args.bn_type)#.half()
#         self.intervener = MLP(self.embed_feat_dim, self.embed_feat_dim, args)#.half()
#         self.class_layer = Classifier(self.embed_feat_dim, self.class_num, type='wn')#.half()
#
#     def get_embed_feat(self, input_imgs):
#         # input_imgs [B, 3, H, W]
#         backbone_feat = self.image_encoder(input_imgs).float()
#         # embed_feat = self.feat_embed_layer(backbone_feat)
#         return backbone_feat
#
#     def forward(self, x, apply_softmax=True):
...
#         x = x.to(self._device)
#         rois = self.image_encoder(x)#.half()
#         rois_c, v = self.feat_embed_layer(rois)
#         y = self.class_layer(rois_c, apply_softmax)
#
#         intervention, int_v, int_rois = self.intervener(rois_c)
...
class COSDA_CLIP(nn.Module):
    def __init__(self, args, checkpint):
        ...
        self.class_num = args.class_num           # shared_class_num + source_private_class_num
        if checkpint == None:
            self.clip_model, _ = clip.load(args.clip_model_name, device="cpu", jit=False)
        else:
             self.clip_model, _ = clip.load(checkpint, device="cpu", jit=False)
        # if args.training:
        #     clip_model.load_state_dict(torch.load(args.clip_model_name))
