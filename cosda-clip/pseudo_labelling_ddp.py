...
import numpy as np
import gc
from utils_me import optimal_assignment
...
def Topk_distance3(args, device, model, dataloader, feature_batch=None, targets=None, new_epoch=True, all_proto=None, test=False):
    ...
    if new_epoch:
        ...
        embed_feat_bank= torch.zeros(args.target_len, args.embed_feat_dim).to(device)
        # print(embed_feat_bank.shape)
        gt_label_bank = torch.zeros(args.target_len).long().to(device)
        pred_cls_bank = torch.zeros(args.target_len, args.known_class+1).to(device)
        class_list = [i for i in range(args.known_class+1)]
        home_loader_iter = iter(dataloader)

        #给每个样本赋予一个长度为K+1的score
        begin_index = 0
        for batch_idx in range(len(dataloader)):
            #加载数据
            data_t, _, target_t, _ = home_loader_iter.next()
            images = data_t.to(device)  #
            # print()
            ...
            if images.size(0) == 1:
                ...
                images = torch.cat((images, images_a.to(device) ), dim=0)
                flag = True
                del temp_iter
                del _
            with torch.no_grad():
                ...
                pred_cls = model.module.class_layer(features_temp, apply_softmax=True)
        ...
        neg_feat_proto = torch.from_numpy(faiss_kmeans.centroids).to(device) #
        ...
        # print('generating with old proto')
        # print('222')
        ...
        psd_label_prior = torch.zeros(feat_proto_simi.shape).scatter(1, psd_label_prior_idxs.unsqueeze(1).cpu(), 1.0).to(device)  # one_hot prior #[N, C]
